...
//
        ...
        Dish poullet = dataRetriever.findDishById(2);


        List<DishOrder> dishOrders = new ArrayList<>();
            DishOrder dishOrder = new DishOrder(1, rizLegume, 2 );

            dishOrders.add(dishOrder);

        System.out.println(dishOrders);
        Order orderToSave = new Order(1, "ORDXX", Instant.now(), dishOrders,PaymentStatusEnum.UNPAID);

        Order findrefernece = dataRetriever.SaveOrder(orderToSave);
        System.out.println(findrefernece);
